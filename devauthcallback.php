...
// Lookup the character details in the DB.
    ...
    $stmt->execute(array(':characterid'=>$characterid,':characterhash'=>$owner));
...
// Fill in character details, if they're not in the DB
    ...
    if (!isset($userdetails)) {
        ...
        $ch = curl_init();
        $lookup_url="https://esi.evetech.net/latest/characters/".$characterid."/";
    ...
    $_SESSION['auth_characterid']=$characterid;
    $_SESSION['auth_id']=$userid;
    $_SESSION['auth_charactername']=$charactername;
    $_SESSION['auth_userdetails']=json_encode($userdetails);
    $_SESSION['auth_characterhash']=$owner;
    session_write_close();
    setcookie('fuzzworkauth', $_SESSION['auth_characterid'], 0, '/', 'www.fuzzwork.co.uk', true, true);
